...
<body>
    ...
    <div class="container">
        <input type="hidden" id="karyawan_id" value="{{ $pegawai->id_karyawan }}">
        ...
        <button class="btn logout" type="submit" onclick=openLogoutPopup()>LOGOUT</button>
    ...
    html5Qr.start(
        ...
        (decodedText) => {
            html5Qr.stop().then(() => {
                ...
                // Ambil waktu mulai shift dari data shift yang dipilih (menggunakan idShift)
                const shift = document.querySelector(`#shift-${idShift}`); // Misalnya, mengambil shift berdasarkan ID
                const waktuShiftMulai = new Date(waktuMasuk.toLocaleDateString() + " " + shift.dataset.jamMasuk);  // Ambil jam masuk dari data shift
